...
### Fixed
- **The scheduler stopped emitting the same grid step twice** — Reported as the subdivision lines doubling up while playing. Following the keyboard's clock, each received quarter note re-anchors the grid, and the catch-up only advanced past *now + 20 ms* while the scheduler had already emitted 200 ms ahead: everything in between was emitted again. Measured over six seconds at 80 bpm on sixteenths: ten duplicates, one per received beat, each a few milliseconds from its twin — two hairlines where there should be one. It also double-struck the click, which only sounds on main beats and goes unnoticed when the click is muted. The catch-up now resumes past the last step actually emitted, with half a period of margin to absorb the phase correction. Zero duplicates after, with or without clock jitter.
- **Main beats land every N steps again** — `anchorStep` carries the musical position precisely so that the clock's per-beat reset of `stepIndex` cannot destroy it, and it was never updated: `followClock` moved the anchor without recording by how much. Invisible in steady state, where the anchor advances by exactly one subdivision and the count stays a multiple by accident; it bites the moment a tempo change at the keyboard makes it jump by anything else. Both fixes are needed for the invariant to hold — either alone leaves it broken.
- **The graph drops the tie line from each note to the plumb line** — It taught nothing: the marker's horizontal position already gives the deviation, and the ruler reads it in note values. In exchange it added, per note played, a horizontal hairline the same weight as a subdivision line, in a neighbouring grey, but up to five times more opaque — so playing densely made the grid appear to double. Tinting it with its note's colour was tried first and made it worse, being more conspicuous rather than less.
